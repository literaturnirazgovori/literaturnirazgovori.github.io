    ...
    (function($) {
        $.fn.html5lightbox = function(options) {
            ...
            if (inst.options.initsocial)
                $("head").append('<link rel="stylesheet" href="' + inst.options.jsfolder + 'icons/css/mhfontello.css" type="text/css" />');
            inst.showing = false;
            ...
            inst.readNodeData = function(node, parentNode) {
                ...
                if (!mediumw)
                    mediumw = inst.checkParentClass(node, parentNode, "mediumwidth");
                if (!mediumw)
                    mediumw = w;
                if (!mediumh)
                    mediumh = inst.checkParentClass(node, parentNode, "mediumheight");
                if (!mediumh)
                    mediumh = h;
                var smallw = inst.checkParentData(node, parentNode, "smallwidth");
                var smallh = inst.checkParentData(node, parentNode, "smallheight");
                if (!smallw)
                    smallw = inst.checkParentClass(node, parentNode, "smallwidth");
                if (!smallw)
                    smallw = mediumw;
                if (!smallh)
                    smallh = inst.checkParentClass(node, parentNode, "smallheight");
                if (!smallh)
                    smallh = mediumh;
                ...
                if (container.length > 0) {
                    ...
                    if (container.find(".lightboxdescription").length > 0)
                        newItem[ELEM_DESCRIPTION] = container.find(".lightboxdescription").text()
                }
            ...
            inst.createMarkup = function() {
                ...
                if (inst.options.showarrowsbottomonsmallscreen) {
                    ...
                    if (inst.options.outsidearrowspos == "bottom")
                        styleCss += " .html5-prev-outside {display:block;top:100%;left:0;right:auto;transform:none;} .html5-next-outside {display:block;top:100%;left:auto;right:0;transform:none;}";
                        ...
                        styleCss += " .html5-prev-outside {display:block;top:50%;left:0;right:auto;transform:translate(0,-50%);} .html5-next-outside {display:block;top:50%;left:auto;right:0;transform:translate(0,-50%);}";
                ...
                if (inst.options.showsocial) {
                    ...
                    $(".html5-social-btn", inst.$lightbox).click(function() {
                        ...
                        if ($(this).hasClass("html5-social-facebook"))
                            ...
                            window.open("mailto:?subject=" + encodeURIComponent(shareTitle) + "&body=" + inst.options.sharetextprefix + encodeURIComponent(shareUrl));
                        return false
                    })
                }
                if (inst.options.isTouch && inst.options.navarrowsalwaysshowontouch && inst.options.navarrowspos == "inside")
                ...
                if (inst.options.closeusefont && (inst.options.fullscreenmode || inst.options.closepos == "topright" || inst.options.closepos == "inside"))
                    inst.$close.append('<div class="mh-icon-close"></div>');
                else
                    inst.$close.append("<img alt='' src='" + (inst.options.fullscreenmode || inst.options.closepos == "topright" ? inst.options.fullscreencloseimage : inst.options.closeimage) + "'>");
            ...
            inst.showNavigation = function() {
                if (!inst.options.shownavigation)
                    return;
                if ((inst.options.isIPhone || inst.options.isAndroid) && inst.options.hidenavigationonmobile || inst.options.isIPad && inst.options.hidenavigationonipad)
                    return;
                ...
                $("body").append("<div id='html5box-html5-lightbox-nav' class='html5-nav' style='display:block;position:" + posType + ";" + posCss + "width:100%;height:" + inst.options.navheight + "px;z-index:9999999;" + (inst.options.navbgcolor ? "background-color:" + inst.options.navbgcolor + ";" : "") + "'>" + "<div class='html5-nav-container' style='position:relative;margin:" + inst.options.thumbtopmargin + "px auto " + inst.options.thumbbottommargin + "px;'>" + "<div class='html5-nav-prev' style='display:block;position:absolute;cursor:pointer;width:" + inst.options.navbuttonwidth + 'px;height:100%;left:0;top:0;background:url("' + inst.options.navarrowsprevimage + "\") no-repeat left center;'></div>" + "<div class='html5-nav-mask' style='display:block;position:relative;margin:0 auto;overflow:hidden;'>" + "<div class='html5-nav-list'></div>" + "</div>" + "<div class='html5-nav-next' style='display:block;position:absolute;cursor:pointer;width:" + inst.options.navbuttonwidth + 'px;height:100%;right:0;top:0;background:url("' + inst.options.navarrowsnextimage + "\") no-repeat right center;'></div>" + "</div>" + "</div>");
                ...
                if (inst.options.shownavcontrol) {
                    ...
                    if (inst.options.navusefont)
                        $(".html5-nav-showcontrol").append('<div class="mh-icon-th-thumb"></div>');
                    else
                        $(".html5-nav-showcontrol").append('<img alt="" src="' + inst.options.navcontrolimage + '"></img>');
            ...
            inst.loadElem = function(elem) {
                ...
                if (inst.options.pushwindowhistory && window.history && window.history.pushState) {
                    ...
                    window.onhashchange = function() {
                        if (!window.location.hash || window.location.hash.indexOf("#html5lightbox_") < 0)
                            inst.finish()
                    }
                }
                if (inst.options.googleanalyticsaccount && window._gaq)
            ...
            inst.getElemSize = function(elem, initW, initH) {
                ...
                if (elem[ELEM_TYPE] == 0) {
                    if (elem[wIndex])
                        ...
                        elemW = inst.options.defaultwidth;
                    if (elem[hIndex])
                        elemH = elem[hIndex];
                    else if (initH) {
                        elemH = initH;
                        ...
                        elemH = inst.options.defaultheight
            ...
            inst.calcElemSize = function(sizeObj, keepratio) {
                ...
                if (strW.charAt(strW.length - 1) == "%")
                    sizeObj.w = winWidth * parseFloat(strW.substring(0, strW.length - 1)) / 100;
                var strH = String(sizeObj.h);
                if (strH.charAt(strH.length - 1) == "%")
                    sizeObj.h = winH * parseFloat(strH.substring(0, strH.length - 1)) / 100;
                ...
                if (inst.options.fullscreenmode && winWidth > inst.options.navarrowsbottomscreenwidth || inst.options.isTouch && inst.options.navarrowsalwaysshowontouch && winWidth > inst.options.navarrowsbottomscreenwidth)
                    w0 -= 64;
                ...
                if (inst.currentElem[ELEM_TYPE] == 0 && keepratio) {
                    ...
                    if (inst.options.resizeleftright && (inst.options.titlestyle == "left" || inst.options.titlestyle == "right") && winWidth > inst.options.sidetobottomscreenwidth) {
                        ratio = (inst.currentElem[ELEM_ORIGINALWIDTH] + 2 * inst.options.bordersize) / inst.currentElem[ELEM_ORIGINALHEIGHT];
            ...
            inst.resizeLightbox = function(elemW, elemH, bAnimate, onFinish, speedZero) {
                ...
                if ((inst.options.transition == "slide" || inst.options.transition == "crossfade") && inst.existingElem >= 0)
                    speed = 0;
                if (typeof speedZero !== "undefined" && speedZero)
                    speed = 0;
            ...
            inst.finish = function() {
                ...
                if (inst.options.enteranimation)
                    inst.$lightboxBox.removeClass("html5box-enter-animated html5box-" + inst.options.enteranimation);
            ...
            inst.showLightboxObject = function(obj) {
                if (obj)
                    inst.showLightbox(obj.type, obj.href, obj.title, obj.width, obj.height, obj.webm, obj.ogg, obj.thumbnail, obj.description, obj.div, obj.originalwidth, obj.originalheight, obj.socialmedia, obj.weblink, obj.weblinktarget, obj.weblinktext, obj.group, obj.mediumwidth, obj.mediumheight, obj.smallwidth, obj.smallheight)
            }
            ;
            ...
            inst.each(function() {
                ...
                if (classoptions) {
                    if ("autoopen"in classoptions)
                        autoopen = classoptions.autoopen;
                ...
                $videoCaption.css({
                    ...
                    if ($videoObj.get(0).textTracks)
                        for (var i = 0; i < $videoObj.get(0).textTracks.length; i++)
                            $videoObj.get(0).textTracks[i].mode = showcaption ? "showing" : "hidden"
                })
            }
                ...
                if ($videoObj.get(0).textTracks) {
                    for (var i = 0; i < $videoObj.get(0).textTracks.length; i++) {
                        ...
                        if ($videoObj.get(0).textTracks[i].activeCues)
                            for (var j = 0; j < $videoObj.get(0).textTracks[i].activeCues.length; j++)
                                $videoObj.get(0).textTracks[i].activeCues[j].line = vttline;
                        if ($videoObj.get(0).textTracks[i].cues)
                            for (var j = 0; j < $videoObj.get(0).textTracks[i].cues.length; j++)
